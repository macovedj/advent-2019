...
const pointsCalc = wire => wire.reduce(
    (acc, curr) => {
        let { currentLocation, turningPoints} = acc;
        if (curr[0] === "U") {
            currentLocation[1] += Number(curr.substr(1))
        } else if (curr[0] === "D") {
            currentLocation[1] -= Number(curr.substr(1))
        } else if (curr[0] === "L") {
            currentLocation[0] -= Number(curr.substr(1))
        } else if (curr[0] === "R") {
            currentLocation[0] += Number(curr.substr(1))
        }
        turningPoints = turningPoints.concat([[...currentLocation]])
        return({currentLocation, turningPoints})
    },
    {currentLocation: [0,0], turningPoints: []}
).turningPoints.reduce(
    (acc, cur) => {
